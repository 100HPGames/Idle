...
namespace Leopotam.EcsProto.QoL
{
...
#endif
            foreach (var fi in target.GetType()
    ...
    [Il2CppSetOption (Option.ArrayBoundsChecks, false)]
#endif
    public sealed class DestroyHereSystem<T> : IProtoInitSystem, IProtoDestroySystem
    ...
    public static class ProtoSystemsExtensions
    {
        ...
        public static IProtoSystems DelHere<T>(this IProtoSystems systems, string worldName = default,
            string pointName = default)
            where T : struct
        {
        ...
        public static IProtoSystems DestroyHere<T>(this IProtoSystems systems, string pointName = default)
            where T : class, IProtoDestroyService
        {
            return systems.AddSystem(new DestroyHereSystem<T>(), pointName);
        }
    }
