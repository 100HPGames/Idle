...
using UnityEditor;
using UnityEngine;
using static CodeBase.Tools.StaticDataLoader.GameConst;
...
namespace CodeBase.Tools.StaticDataLoader
{
    public abstract class ParseHelper
    {
        ...
        public static object WriteDataInListByType(Type t, Dictionary<string, List<string>> data,
            Dictionary<string, string> allCsvData, string owner = default)
        {
            Array instances;
//          Debug.Log(t.Name);
            ...
            if (data[Key].Any(val => val.Contains(ListIndex)))
            {
                ...
                {
                    if (keyedList.Value.IndexOf(value) == 0)
                        continue;

                    /*
                     List<string> owners = value.Split(' ').ToList();
